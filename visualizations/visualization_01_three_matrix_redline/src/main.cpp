#include <M5Chain.h>
#include <M5Unified.h>
#include "../../visualization_03_lower_matrix_blocks/matrix_config.h"
...
uint8_t activeSectionsForRpm(uint16_t currentRpm) {
  const uint8_t stage = ShiftlightConfig::stageForRpm(currentRpm);
  return stage == 4 ? kSectionCount : stage;
}
...
void renderShiftlight(uint16_t currentRpm, uint8_t activeSections) {
  if (currentRpm >= ShiftlightConfig::kShiftPoints.redline) {
  ...
  for (uint8_t section = 0; section < activeSections; ++section) {
    ...
    const uint16_t color = currentRpm >= ShiftlightConfig::kShiftPoints.allRed
                               ? kRed
                               : (section < 3 ? kGreen : kYellow);
...
void renderScreen(uint16_t currentRpm, uint8_t activeSections) {
  ...
  const bool redline = currentRpm >= ShiftlightConfig::kShiftPoints.redline;
  const bool allRed = currentRpm >= ShiftlightConfig::kShiftPoints.allRed;
...
void setup() {
  ...
  M5.Display.drawString("VISUAL 01: 3 RGB", M5.Display.width() / 2, 64);

  Serial.println("Shiftlight visualisation 01: three-matrix redline");
...
void loop() {
  ...
  if (rpm >= ShiftlightConfig::kShiftPoints.redline &&
      now - lastRedFlashMs >= kRedFlashHalfPeriodMs) {
    updateVisualisation();
    lastRedFlashMs = now;
  }
